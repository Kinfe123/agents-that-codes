...
## Why It Matters
Studio Config matters because it is part of how Coding Agent Template turns repository behavior into a usable developer or operator workflow. This page gives readers the mental model first, then uses implementation evidence as supporting context.
## Core Concepts
- Studio Config
- configuration type
- runtime options
...
- `lib/sandbox/config.ts` - Evidence file that contributes to the implementation understanding for this page.
- `next.config.ts` - Evidence file that contributes to the implementation understanding for this page.
- `package.json` - Evidence file that contributes to the implementation understanding for this page.

### Source Details
- `lib/sandbox/config.ts`: language TypeScript; exports `createAuthenticatedRepoUrl`, `createSandboxConfiguration`, `validateEnvironmentVariables`; env `ANTHROPIC_API_KEY`, `CURSOR_API_KEY`, `GITHUB_TOKEN`, `OPENAI_API_KEY`
